...
```{r}
head(veg[,names(a[a==1])][,1:6])
head(veg[,names(a[a==1])][,7:12])
```
...
```{r}
head(veg.chem.48[,1:3])
head(veg.chem.48[,4:5])
```

## Toxicity Level for Restricted Use Chemicals {.smaller}
```{r}
