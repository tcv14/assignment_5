...
- The majority part of the analysis is based on temperature data recorded at noon. 
    ...
    + Note that methods of recording data changes over time so we adjust our methods of data selection accordingly
    
        + for data before 2009: we select temperatures recorded exactly at noon (ie. 12:00 PM)
        + for data in/after 2009: we select temperatures recorded slightly before noon (ie. 11:50 AM)
...
- *Note: We conducted the one-way test which not assuming equal variances, and this condition is assumed for all one-way tests in the following sections.*
...
- Due to the nature of the sampled data (ie. only one observation per day at noon), it can be possible that the temperatures we are recording are higher because the sun is almost above the buoy at noon time. If we had picked the observations at night, the temperatures would probably be lower.
